...
export default function Hero({ topicCount, phraseCount }: HeroProps) {
  return (
    <div className="bg-surface border border-border rounded-[var(--radius-lg)] p-6 sm:p-8 pb-7 mb-6 relative overflow-hidden shadow-sm">
      <div
        className="absolute top-0 right-0 w-[220px] h-[220px] pointer-events-none"
        style={{
          background: 'radial-gradient(circle at top right, color-mix(in srgb, var(--accent) 15%, transparent) 0%, transparent 70%)',
        }}
      />
      ...
      <h1 className="text-[22px] sm:text-[26px] font-semibold text-text-primary tracking-tight leading-tight mb-1.5">
        Deutsch <span className="text-accent">Phrasebook</span>
      </h1>
      <p className="text-sm text-text-secondary leading-relaxed max-w-[480px]">
        <T k="14 essential topics for the Goethe A1 exam — real phrases, grammar tips, and natural fluency practice." />
      </p>
      <div className="flex items-center justify-between mt-5 pt-5 border-t border-border flex-wrap gap-3">
        <div className="flex items-center gap-4 flex-wrap">
          <div className="flex items-center gap-1.5 text-xs text-text-secondary">
            ...
            <strong className="text-text-primary">{topicCount}</strong> <T k="topics" />
          </div>
          <div className="flex items-center gap-1.5 text-xs text-text-secondary">
            <svg className="w-[15px] h-[15px] text-accent" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" />
            </svg>
            <strong className="text-text-primary">{phraseCount}+</strong> <T k="phrases" />
          </div>
          <div className="flex items-center gap-1.5 text-xs text-text-secondary">
            <svg className="w-[15px] h-[15px] text-accent" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
