...
const topicsMatch = html.match(/const topics = ([\s\S]*?);\s*\n\s*\/\/ ── Bangla/);
...
const groupTitleCode = html.substring(html.indexOf('{', mapStart), html.indexOf('}', mapStart) + 1);
const bnGroupTitles = eval(`(${groupTitleCode})`);

// Write output files
const outDir = resolve(__dirname, '../src/data');
writeFileSync(resolve(outDir, 'topics.json'), JSON.stringify(cleanTopics, null, 2));
writeFileSync(resolve(outDir, 'bn-translations.json'), JSON.stringify(bnTranslations, null, 2));
writeFileSync(resolve(outDir, 'bn-group-titles.json'), JSON.stringify(bnGroupTitles, null, 2));
